...
class Parser():
    ...
    async def reply_parser(post, topic):
        ...
        if match_quote:
            ...
            quote_date = datetime.strptime(
                quote_date_str, '%Y-%m-%d %H:%M')
            quoted_id = topic.find_post_id_by_author_id_and_date(quoto_uid, quote_date)
            if quoted_id is None:
                quoted_id = await mongo.search_posts_by_author_and_date(topic.tid, quoto_uid, quote_date)
            if quoted_id is not None:
                topic.posts[quoted_id].quoted_by.append(post.pid)
                post.quote_to.append(quoted_id)
        ...
        if match_reply:
            logging.debug('回复')
            replied_post_uid = match_reply.groups()[0]
            replied_post_uname = match_reply.groups()[1]
            replied_post_date_str = match_reply.groups()[2]
            post.content = match_reply.groups()[3]
            # print(post_uname, post_time)
            replied_post_date = datetime.strptime(
                replied_post_date_str, '%Y-%m-%d %H:%M')
            replied_post_id = topic.find_post_id_by_author_id_and_date(replied_post_uid, replied_post_date)
            if replied_post_id is None:
                replied_post_id = await mongo.search_posts_by_author_and_date(topic.tid, replied_post_uid, replied_post_date)
            if replied_post_id is not None:            
                topic.posts[replied_post_id].replied_by.append(post.pid)
                post.reply_to.append(replied_post_id)
                replied_post_content = topic.posts[replied_post_id].content
                post_html += f'<blockquote><p>{replied_post_uname}:{replied_post_content}</p></blockquote>{post.content}'
            return post_html
        return post.content
    ...
    async def page_parser(soup, page, topic: Topic):
        ...
        for i, str_post_content in enumerate(str_post_contents):
            ...
            post.up_counts = pattern_up_counts.search(
                str_up_counts[i].string).group(1)
            post.content = await Parser.img_parser(topic.title, post.content)
            post.content = await Parser.reply_parser(post, topic)
